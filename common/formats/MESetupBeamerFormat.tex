...
{ %Make a graphic on the title page
	\setbeamertemplate{title page}{%
 % % We are making the title page "by hand"
	}
}
{ % Make a text-only title page
    \setbeamertemplate{title page}{%
	...
	{\parbox[t]{.8\paperwidth}{\raggedright\usebeamerfont{subtitle}\textcolor{black}{\scriptsize \copyright \LectureAuthor\ \\\LectureRevision\ (using\ \MERevision)}}};
...
\setbeamertemplate{footline}{\raisebox{-2ex}{\pgfuseimage{textlogo}}
  \usebeamerfont{date in head/foot}\insertshortdate{}\hfill
  \insertframenumber{}/\inserttotalframenumber\raisebox{-2ex}{\insertlogo}}
...
{
 \usebackgroundtemplate{\includegraphics[width=\paperwidth,keepaspectratio]{fig/CoverIllustration.jpeg}}
  \begin{frame}[plain]
	\begin{tikzpicture}[remember picture,overlay]
	\node[anchor=east]
	...
	{\parbox[t]{.7\paperwidth}{\raggedleft\usebeamerfont{subtitle}{    \begin{mybox}[]
	       {  \bfseries\textcolor{\TitlePageColor}
	       \LectureAuthor\\\small\bfseries\textcolor{\TitlePageColor}\LecturePublisher}
	    \end{mybox}
	}}};
	\ifx\DisableLogos\undefined
	  %% We can use logos, either custom logos or default ones
		...
		{
        	\node[anchor=west]
	        at ([yshift=.75cm,xshift=-.18cm]current page.south west) (logo)
			{\includegraphics[width=2cm]{../../common/images/LatexTalk.png}\includegraphics[width=2cm]{fig/TextLogo.jpg} \raisebox{.5ex}
			{\tiny{\ \LectureRevision\ (using\ \MERevision}) \ \copyright \LectureAuthor}};
		}
		{ % No user logo, use the default one
			\node[anchor=west]
			at ( [yshift=1cm]
			current page.south west) (logo)
			{\includegraphics[width=2cm]{../../common/images/LatexTalk.png}\raisebox{.5ex}
						{\tiny{\ \LectureRevision\ (using\ \MERevision}) \ \copyright \LectureAuthor}};
		}
...
\newcommand\MEbackmatter{
	...
	\ifx\DisableIndex\undefined		
       \begin{frame}[allowframebreaks]{Tárgymutató}
		\printindex
	   \end{frame}
	\else 
 ...
 %% Printing bibliography is allowed
	...
	\else
      ...
      {
      \begin{frame}[allowframebreaks]{Felhasznált források}
		\printbibliography
	  \end{frame}
      }
	  {
      ...
      {
      \begin{frame}[allowframebreaks]{Used sources}
		\printbibliography
	  \end{frame}
      }
	  {
